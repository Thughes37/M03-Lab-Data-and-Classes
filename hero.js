...
function selectionChanged() {
  ...
  // Display the selected hero and villain's details
  if (selectedHero && selectedVillain) {
    const battleResult = selectedHero.battle(selectedVillain);
    const winnerAlias = battleResult ? selectedHero.alias : selectedVillain.alias;
    
    document.querySelector("#winner").textContent = `Winner: ${winnerAlias}!`;
  } else {
    document.querySelector("#winner").textContent = "Select a hero and a villain";
  }
}
